...
def check_research_status():
    ...
    try:
        response = requests.get(
            f"http://localhost:8000/api/research/{st.session_state.research_task_id}",
                ...
                if report_length > 10:  # Make sure we have actual content
                    ...
                    try:
                        fresh_response = requests.get(
                            f"http://localhost:8000/api/research/{st.session_state.research_task_id}?refresh=true",
                            timeout=15
                        )
        ...
        if st.session_state.research_task_id:
            ...
            if st.session_state.research_report:
                ...
                if st.button("🔍 Debug Report", key="debug_report"):
                    try:
                        debug_response = requests.get("http://localhost:8000/api/debug/research", timeout=10)
            ...
            if not st.session_state.research_report:
                try:
                    ...
                    final_response = requests.get(
                        f"http://localhost:8000/api/research/{st.session_state.research_task_id}?final=true",
                        timeout=20
                    )
